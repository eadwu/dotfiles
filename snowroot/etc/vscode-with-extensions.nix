...
in
  ...
  runCommand "${wrappedPkgName}-with-extensions-${wrappedPkgVersion}" {
    ...
    if [ ! -d "$out/share" ]; then mkdir "$out/share"; fi
    if [ ! -d "$out/share/applications" ]; then mkdir "$out/share/applications"; fi
    if [ ! -d "$out/share/pixmaps" ]; then mkdir "$out/share/pixmaps"; fi
